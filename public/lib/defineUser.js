...
var _loop = function _loop(i) {
  buttonDefineUser[i].addEventListener('click', function () {
    console.log(role[i]);
    defineUser('/defineUser', username[i], role[i]).then(function (res) {
...
for (var i = 0; i < buttonDefineUser.length; i++) {
  _loop(i);
}
...
function defineUser(url, user, role) {
  var body = 'username=' + encodeURIComponent(user.innerHTML) + '&role=' + encodeURIComponent(role.value);
  console.log(body);
